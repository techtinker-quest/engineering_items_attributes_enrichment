...
# --- GitHub API Mode Logic (Omitted for brevity, remains the same) ---
...
def traverse_github_repo(
    ...
    for item in tree_data:
        ...
        commit_date, committer = fetch_file_commit_details(owner, repo, path, pat)
        ...
        if Path(path).suffix.lower() in CODE_FILE_TYPES:
            try:
                ...
                ast_notes = "AST failed (raw fetch error, timeout, or large file)"
...
# --- Local File System Mode Logic (Omitted for brevity, remains the same) ---
...
def traverse_local_dir(root_path: str, exclude_types: list[str]) -> list[dict]:
    ...
    for entry in root_path_obj.rglob("*"):
        ...
        if entry.is_file():
            ...
            try:
                ...
                last_mod_ts = datetime.fromtimestamp(stat_result.st_mtime).isoformat()
            except OSError as e:
                logging.warning(
            ...
            if entry.suffix.lower() in CODE_FILE_TYPES:
                file_content = ""
                try:
                    file_content = entry.read_text(encoding="utf-8", errors="strict")
                    classes, functions, methods, ast_notes = analyze_python_file(
                        file_content
                    )
                    ...
                    ast_notes = (
                        f"AST Error: General Read/Parse Error ({e.__class__.__name__})"
                    )
